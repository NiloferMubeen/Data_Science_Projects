...
# git_url_to_clone = !git clone https://github.com/PhonePe/pulse.git

#Aggregate TRANSACTION

def trans_agg():
        agg_path1='C://Users/Welcome/pulse/data/aggregated/transaction/country/india/state'
        Agg_state_list =os.listdir(agg_path1)  # list of 36 states

        columns ={'State':[], 'Year':[],'Quarter':[],'Transaction_type':[], 'Transaction_count':[], 'Transaction_amount':[]}

        for i in Agg_state_list:
            p_i = agg_path1 +'/'+ i 
            Agg_yr = os.listdir(p_i) #list of 5 years
            
            for j in Agg_yr:
                p_j = p_i +'/'+ j
                Agg_yr_list=os.listdir(p_j) # list of json files for every quarter
                
                for k in Agg_yr_list:
                        p_k =p_j + '/'+ k
                        Data=open(p_k,'r')
                        D=json.load(Data)  # json to python dict
...
with cl2:
        st.markdown('<img style="float: center;" src="https://miro.medium.com/v2/resize:fit:400/format:webp/1*WG_NcuKQE6uRQmGAhJ89Mg.png" />', unsafe_allow_html=True)
...
# Other visualisations and analysis
                        
menu = option_menu(None,["Categories","States","Districts","Postal Codes","Mobile Brands","Yearly Trends"], orientation="horizontal")
if menu == "Categories":
        ...
        with col1:
               ...
               st.table(user_data)
        with col2:
               fig = px.pie(user_data, values='Total_transactions', names="Categories",color_discrete_sequence=px.colors.sequential.RdBu,hole = 0.5)
        ...
        state = pd.read_sql_query('select "State" as "States" ,sum("Transaction_count") as "Total_Transactions" from agg_transac group by "State" order by "Total_Transactions" desc limit(10)',con =engine)
        col1,col2 = st.columns(2)
        with col1:
                st.markdown("<h4 style='text-align: center; color: #063F27;'>Top 10 States </h4>", unsafe_allow_html=True)
                st.table(state)
        with col2:
                fig = px.pie(state, values="Total_Transactions", names="States",color_discrete_sequence=px.colors.sequential.speed)
                fig.update_layout(legend=dict(
                                yanchor="top",
                                y=0.99,
                                xanchor="left",
        ...
        dist = pd.read_sql_query('select "District_name" as "Districts",sum("Top_Transac_amount") as "Total_amount" from top_dis_transac group by "District_name" order by "Total_amount" desc limit(10)',con = engine)
        col1,col2 = st.columns(2)
        with col1:
                st.markdown("<h4 style='text-align: center; color: #063F27;'>Top 10 Districts </h4>", unsafe_allow_html=True)
                st.table(dist)
        with col2:
                fig = px.pie(dist, values="Total_amount", names="Districts",color_discrete_sequence=px.colors.sequential.dense)
                fig.update_layout(legend=dict(
                                yanchor="top",
                                y=0.99,
                                xanchor="left",
...
elif menu == "Postal Codes":
        pin = pd.read_sql_query('select "Pincode" as "Postal Codes",sum("Top_Transac_amount") as "Total_amount" from top_pin_transac group by "Pincode" order by "Total_amount" desc limit(10) ',con=engine)
        col1,col2 = st.columns(2)
        with col1:
                st.markdown("<h4 style='text-align: center; color: #063F27;'>Top 10 Postal Codes </h4>", unsafe_allow_html=True)
                st.table(pin)
        with col2:
                fig = px.pie(pin, values="Total_amount", names="Postal Codes",color_discrete_sequence=px.colors.sequential.matter)
                fig.update_layout(legend=dict(
                                yanchor="top",
                                y=0.99,
                                xanchor="left",
                ...
                st.plotly_chart(fig, use_container_width=False)
elif menu == "Mobile Brands":
        mob = pd.read_sql_query('select "Mobile_brand",sum("User_count") as "Users" from agg_user group by "Mobile_brand" order by "Users" desc limit (10)',con=engine)
        col1,col2 = st.columns(2)
        with col1:
                st.markdown("<h4 style='text-align: center; color: #063F27;'>Top 10 Mobile Brands </h4>", unsafe_allow_html=True)
                st.table(mob)
        with col2:
                fig = px.bar(mob, x="Users", y="Mobile_brand",color_discrete_sequence=px.colors.sequential.Rainbow,orientation='h')
                st.plotly_chart(fig, use_container_width=False)
        ...
        with col1:
                ...
                opt = st.selectbox("Chart Type",('Line','Bar',"Area","Scatter"),index=0)
                ...
                if opt == 'Line':
                        ...
                        fig = px.area(yearly_trend, x="Year", y='Total Transactions')
                        st.plotly_chart(fig, use_container_width=True)
                elif opt == 'Scatter':
                        fig = px.scatter(yearly_trend, x="Year", y='Total Transactions',color= "Year",size = "Total Transactions",hover_data=['Total Transactions'])
                        st.plotly_chart(fig, use_container_width=True)
        with col2:
                ...
                opt1 = st.selectbox("Chart Type",('Line Chart','Bar Chart','Area Chart','Scatter plot'),index=0)
                if opt1 == 'Line Chart':
                        ...
                        st.plotly_chart(fig, use_container_width=True)
                elif opt1 == 'Area Chart':
                        fig = px.area(user_trend, x="Year", y='User_count',color = 'Year',hover_data=['User_count'])
                        st.plotly_chart(fig, use_container_width=True)
                elif opt1 == 'Scatter plot':
                        fig = px.scatter(user_trend, x="Year", y='User_count', color="Year",
                 size='User_count', hover_data=['User_count'])
                        st.plotly_chart(fig, use_container_width=True)
...
st.markdown("<h4 style='text-align: center; color: #063F27;'>State Level Transactions </h4>", unsafe_allow_html=True)
...
with x1:
        fig = px.bar(data, x="Quarter",y="Transactions_count",color='Quarter',text_auto='.2s')
